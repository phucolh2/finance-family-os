...
export const useLiquidityBreakdown = (mode: 'monthly' | 'cumulative' = 'monthly', customPeriodKey?: string) => {
  ...
  const liquidityBreakdownData = useMemo(() => {
    ...
    const addFlexibleEvent = (groupId: string, event: any, impact: number, type: 'oneTime' | 'trackA' | 'trackB') => {
      ...
      if (!flexibleByGroup[groupId]) flexibleByGroup[groupId] = { oneTime: 0, trackA: 0, trackB: 0 };
      flexibleByGroup[groupId][type] += impact;
      
      if (!flexibleEventsByGroup[groupId]) flexibleEventsByGroup[groupId] = [];
      
      const existing = flexibleEventsByGroup[groupId].find(e => e.id === event.id && e.type === type);
      if (existing) {
    ...
    (state.lifeEvents || []).forEach(event => {
      ...
      // Calculate recurring active months
      ...
      // 2) Track A (event.recurringMonthlyImpact) -> deduct from event.spendingCategory
    ...
    return expenseTree.map((g: any) => {
      ...
      const flexData = flexibleByGroup[groupId] || { oneTime: 0, trackA: 0, trackB: 0 };
      const flexibleEvents = flexibleEventsByGroup[groupId] || [];
      ...
      const oneTimeIncome = flexibleEvents
        ...
        if (mode === 'monthly') {
          totalBudget = targetDb?.budgetAmounts?.[groupId] || 0;
          if (targetDb?.actualExpenseByGroup && typeof targetDb.actualExpenseByGroup[groupId] === 'number') {
            totalActual = targetDb.actualExpenseByGroup[groupId];
          ...
          // Note: totalActual now represents purely the base 'Thường xuyên' because the UI splits the event impact.
        } else {
        totalBudget = cumulativeExpenseData?.summaryByGroup?.[groupId]?.totalBudget || 0;
        totalActual = cumulativeExpenseData?.summaryByGroup?.[groupId]?.totalRegularActual || 0;
      }
