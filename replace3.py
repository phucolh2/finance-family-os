...
# Replace hashtag label
content = content.replace('Nhóm (Hashtag)', 'Nhóm')

# We need to add the imports at the top
if 'LifestyleFundCard' not in content:
...
'''
    content = content.replace('import { HelpTooltip } from \'../ui/HelpTooltip\';', imports + 'import { HelpTooltip } from \'../ui/HelpTooltip\';')

# Add variant prop to interface
if 'variant?: \'portfolio\' | \'savings\' | \'reserves\' | \'lifestyle\';' not in content:
    content = content.replace('  emptyStateDescription: string;\n}', '  emptyStateDescription: string;\n  variant?: \'portfolio\' | \'savings\' | \'reserves\' | \'lifestyle\';\n}')

# Add variant to props destructuring
if 'variant = \'portfolio\'' not in content:
    content = content.replace('  emptyStateDescription\n})', '  emptyStateDescription,\n  variant = \'portfolio\'\n})')

start_str = '            return (\n              <div key={fund.id} className="border border-family-accent/20'
end_str = '              </div>\n            );\n          })}\n        </div>\n      )}\n    </div>\n  );\n};\n'
...
'''
    ...
    new_content = content[:start_idx] + replacement + end_str
    with open('src/components/portfolio/SinkingFundModule.tsx', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Success")
else:
    print(f"No match found: start_idx={start_idx}, end_idx={end_idx}")
