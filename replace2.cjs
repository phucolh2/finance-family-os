const replacement = `            const renderCashflowDetails = (fund: any) => (
...
const fs = require('fs');
let content = fs.readFileSync('src/components/portfolio/SinkingFundModule.tsx', 'utf8');

const lines = content.split('\n');

// startLine is 536 (index 535)
// endLine is 1022 (index 1021)

const before = lines.slice(0, 535);
const after = lines.slice(1022);

const newLines = [...before, replacement, ...after];
fs.writeFileSync('src/components/portfolio/SinkingFundModule.tsx', newLines.join('\n'));
